...
2 # F(MSY) for projections: 1= set to F(SPR); 2=calc F(MSY); 3=set to F(Btgt) or F0.1; 4=set to F(endyr) 
0.48 # SPR target (e.g. 0.40)
0.48 # Relative Biomass target (e.g. 0.40)
...
0.4 # B(targ), Control rule Biomass level for constant F (as frac of Bzero, e.g. 0.40); (Must be > the no F level below) 
0.2 # B(lim), Control rule Biomass level for no F (as frac of Bzero, e.g. 0.10) 
...
# add lines here for rebuilding and over-ride forecasted catch or F
-9999 1 1 0 
#
